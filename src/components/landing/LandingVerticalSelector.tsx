...
type LandingVerticalSelectorProps = {
  value: BusinessVertical;
  onChange: (vertical: BusinessVertical) => void;
  className?: string;
};

export default function LandingVerticalSelector({
  value,
  onChange,
  className = "",
}: LandingVerticalSelectorProps) {
      ...
      {BUSINESS_VERTICALS.map((vertical) => {
        ...
        return (
          <button
            ...
            aria-label={`${isEnglish ? "Select" : "Seleccionar"} ${copy.label}`}
